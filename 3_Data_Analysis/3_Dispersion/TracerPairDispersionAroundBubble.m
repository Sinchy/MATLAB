function [R, disp_matrix]  = TracerPairDispersionAroundBubble(tracer_pair, tracer_tracks, initial_separation)
...
for i = sample_rate : sample_rate : num_pair
    ...
    track2(track2(:,4) < start_frame | track2(:,4) > end_frame, :) = [];
     if exist('initial_separation', 'var')
          ...
          IS = norm(track1(1, 1:3) - track2(1, 1:3), 2);
    ...
    len = size(disp_vec, 1);
    disp_matrix(round(i/sample_rate), 1 : len) = (disp_sca - disp_sca(1)) .^ 2;
