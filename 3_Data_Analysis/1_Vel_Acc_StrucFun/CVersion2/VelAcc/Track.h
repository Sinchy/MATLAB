...
public:
	...
	// return the loaction along one axis
	vector<double> PosAlongAxis(int axis);
	...
	//void LoadTrackFromTXT(string path);

	void SetPos(std::deque<Position> p) { pos = p; };
	void SetVel(std::deque<Position> v) { vel = v; };
	void SetAcc(std::deque<Position> a) { acc = a; };
	void SetTime(std::deque<int> t) { time = t; };

	Position GetPos(int frame) { return pos[frame]; };
	Position GetVel(int frame) { return vel[frame]; };
	Position GetAcc(int frame) { return acc[frame]; };
