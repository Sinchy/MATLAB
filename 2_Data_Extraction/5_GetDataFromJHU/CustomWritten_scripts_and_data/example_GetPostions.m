...
vel = getVelocity(authkey, dataset, start_time, Lag4, ...
...
% end_time = 8;
% dt = 0.05;
% points = newpoints(newpoints(:, 4) == 68, 1:3)';
% start_time = start_time + dt * 68;
% j = 69;
for time = start_time:dt:end_time
%     parfor_progress(j);
    points = getPosition(authkey, dataset, time, time + dt, dt, Lag4, npoints, points);
    ...
    vel = getVelocity(authkey, dataset, time + dt, Lag4, ...
                              PCHIPInt, npoints, points);
...
%     end
    j = j + 1
    if ~mod(j, 10)
        save('simulation_data.mat', 'newpoints', 'time','j', '-v7.3');
    end
end
