...
function result = getTemperatureGradient(authToken, dataset, time, spatialInterpolation, ...
                                      temporalInterpolation, npoints, points)			     
%
%     Retrieve Temperature gradient for specified 'time' and 'points'
%   
%     Input:
%       authToken = (string)
%       dataset = (string)
...
resultStruct =  GetTemperatureGradient (obj, authToken, dataset, time, ...
		spatialInterpolation, ...
		temporalInterpolation, ...
		points);
    
result = getVector(resultStruct.GetTemperatureGradientResult.Vector3);
