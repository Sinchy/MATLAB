...
function GetTemperatureHessianResult = GetTemperatureHessian(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%GetTemperatureHessian(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%
...
%   
%     Output:
%       GetTemperatureHessianResult = (ArrayOfTemperatureHessian)
...
response = callSoapService( ...
    ...
    soapMessage);
GetTemperatureHessianResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetTemperatureHessianResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetTemperatureHessianResult.faultcode, ...
        GetTemperatureHessianResult.faultstring);
end