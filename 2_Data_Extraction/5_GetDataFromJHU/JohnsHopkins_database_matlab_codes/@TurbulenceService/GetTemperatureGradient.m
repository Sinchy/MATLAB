...
function GetTemperatureGradientResult = GetTemperatureGradient(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%GetTemperatureGradient(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
%
...
%   
%     Output:
%       GetTemperatureGradientResult = (ArrayOfVector3)
...
response = callSoapService( ...
    ...
    soapMessage);
GetTemperatureGradientResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetTemperatureGradientResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetTemperatureGradientResult.faultcode, ...
        GetTemperatureGradientResult.faultstring);
end