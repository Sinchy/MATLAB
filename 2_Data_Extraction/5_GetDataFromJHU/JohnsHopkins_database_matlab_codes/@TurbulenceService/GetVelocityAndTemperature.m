...
function GetVelocityAndTemperatureResult = GetVelocityAndTemperature(obj,authToken,dataset,time,spatialInterpolation,temporalInterpolation,points)
...
response = callSoapService( ...
    ...
    soapMessage);
GetVelocityAndTemperatureResult = parseSoapResponse(response);

% Fault message handling
if isfield(GetVelocityAndTemperatureResult, 'faultstring')
    error('faultcode: %s\nfaultstring: %s\n', ...
        GetVelocityAndTemperatureResult.faultcode, ...
        GetVelocityAndTemperatureResult.faultstring);
end