function [camParaCalib, particles_info, good_particles, good_tracks] = VolumeSelfCalibV0(tracks, view_size, image_path, start_frame, calib_path, load_file)
if ~exist('load_file', 'var')
    % due to the change of track format, here we should transform the new
    % format into the old format on which this code was based 
    d = size(tracks);
    if length(d) < 3
        tracks = TracksFormatTransform(tracks);
    end
    % Pick good tracks
    good_tracks = PickGoodTracks(tracks, view_size);

    % Choose 4000 particles evenly 
    good_particles = PickParticleEvenly(good_tracks, 5000);
    ...
    warning('on','all');
    save([image_path 'VSCdata.mat'], 'good_tracks', 'good_particles', 'particles_info');
else
   load([image_path 'VSCdata.mat']);
end

    fig = figure;
%     PlotTracksV0(fig, good_tracks, 'g.');
    hold on
...
CalibMatToTXT(camParaCalib, [image_path 'VSC_Calib_new.txt']);
save([image_path 'VSC_Calib_new.mat'], 'camParaCalib');
end
...
x_min = min(min(tracks(:,:,1))); x_max = max(max(tracks(:,:,1)));
y_min = min(min(tracks(:,:,2))); y_max = max(max(tracks(:,:,2)));
z_min = min(min(tracks(:,:,3))); z_max = max(max(tracks(:,:,3)));
even_indicator = ones(1, num_seg ^ 3);
select_label = zeros(size(tracks, 1), size(tracks, 2));
...
for i = 1 : num_seg
    for j = 1 :  num_seg
        for k = 1 : num_seg
            ...
            [index_tracks, index_frames] = find(tracks(:, :, 1) > x_lo & tracks(:, :, 1) < x_up & ...
                tracks(:, :, 2) > y_lo & tracks(:, :, 2) < y_up & ...
                tracks(:, :, 3) > z_lo & tracks(:, :, 3) < z_up);
            total_num = length(index_tracks);
            if total_num > num_particle_seg
                ...
                index = 1 : total_num;
                index_selected = datasample(index, num_particle_seg);
                for l = 1 : num_particle_seg
                    select_track = index_tracks(index_selected(l));
                    select_frame = index_frames(index_selected(l));
                    if (tracks(select_track, select_frame, 1) ~= 0 && tracks(select_track, select_frame, 2) ~= 0 ...
                && tracks(select_track, select_frame, 3) ~= 0) % to make sure it is not a zero point
                        select_label(select_track, select_frame) = 1; 
                    end
            ...
            elseif total_num > 0
                ...
                for l = 1 : total_num
                    select_track = index_tracks(l);
                    select_frame = index_frames(l);
                    if (tracks(select_track, select_frame, 1) ~= 0 && tracks(select_track, select_frame, 2) ~= 0 ...
                && tracks(select_track, select_frame, 3) ~= 0) % to make sure it is not a zero point
                        select_label(select_track, select_frame) = 1; 
                    end
                ...
                even_indicator((i - 1) * num_seg * num_seg + (j - 1) * num_seg + k) = ...
                        total_num / num_particle_seg;
            end
        end
    end
end
...
while num_left > 0
    select_index = datasample(1 : size(select_label, 1) * size(select_label, 2), num_left);
    for i = 1 : num_left
        [select_track, select_frame] = ind2sub(size(select_label), select_index(i));
        if select_label(select_track, select_frame) == 0
            if (tracks(select_track, select_frame, 1) ~= 0 && tracks(select_track, select_frame, 2) ~= 0 ...
                && tracks(select_track, select_frame, 3) ~= 0)
                select_label(select_track, select_frame) = 1;
                num_left = num_left - 1;
            end
        end
...
[index_tracks, index_frames] = find(select_label == 1);
good_particles = zeros(length(index_tracks), 4);
for i = 1 : length(index_tracks)
   good_particles(i, :) = [reshape(tracks(index_tracks(i), index_frames(i), :), 1, 3), index_frames(i)];
end
end
...
fmin_options.MaxFunEvals = 6000; %350;
