...
section_no = extractBetween(dirr, 'S', '/');
if ~exist('totalImgs', 'var')
    a = dir([dirr 'C001H001S000' section_no{1} '/*.tif']);
    totalImgs = numel(a) - 20; 
end
...
for cam = start : ncams
    ...
    num = 0;
    camdir = [dirr 'C00' num2str(cam) 'H001S000' section_no{1} '/'];
    for I = 1:totalImgs_for_background
        if (mod(I,10) == 0)
            ...
            imgdir = [camdir 'C00' num2str(cam) 'H001S000' section_no{1} '0' num2str(I,'%05.0f') '.tif'];
            img = imread(imgdir);
