...
if ~exist('totalImgs', 'var')
    a = dir([dirr 'C001H001S0001/C00*.tif']);
    totalImgs = numel(a); 
end
%% Inputs
...
for cam = 1 : ncams
    num = 0;
    camdir = [dirr 'C00' num2str(cam) 'H001S0001/'];
    for I = 1:totalImgs_for_background
        if (mod(I,20) == 0)
            imgdir = [camdir 'C00' num2str(cam) 'H001S00010' num2str(I,'%05.0f') '.tif'];
            img = imread(imgdir);
...
for cam = 1:ncams
    ...
    while h_end > 2 || h_end == 0 
        imgdir = [camdir 'C00' num2str(cam) 'H001S0001000001.tif'];
        img = imread(imgdir);
