function [shake_point, intensity, error] = ShakeTheBox(active_long_tracks, image_dir)
%     [~, frame_num, ~] =  size(active_long_tracks);

    frame_num = max(active_long_tracks(: ,4));
    
    [est_point, error] = Prediction(active_long_tracks);
    image_path = strings(1,4);
    for i = 1 : 4
%         image_path(i) = fullfile([image_dir 'cam' num2str(i) '/cam' num2str(i) 'frame' num2str(frame_num + 1,'%04.0f') '.tif']);
        image_path(i) = fullfile([image_dir 'cam' num2str(i) '/cam' num2str(i) 'frame' num2str(frame_num + 1,'%05.0f') '.tif']);
    end
    ...
    for i = 1 : loop_num
        shift = shift / 2;
        [shake_point, intensity] = Shake(shake_point, intensity, image_path, shift, project_size);   
    end
end

function [shake_point, intensity] = Shake(est_point, intensity,  image_path, shift, project_size) 
%% get the residual image
    % get the origin image
    origin_img = zeros(4, 1024, 1024);
    for i = 1 : 4
        origin_img(i, :, :) = uint8(imread(convertStringsToChars(image_path(i))));
    end
    % get the reprojected image
    [project_img, xc, yc] = ProjectImage(est_point, project_size * 2); % double size of reprojecting area
    % get the residual image
    ...
    for i = 1 : 1
        ...
        augment_img = GetAugmentResImage(residual_img, est_point(i, :), project_size * 2, xc(i, :), yc(i, :)); % using the origin particle as the projected center
        shake_point(i, :) = Shaking(augment_img, est_point(i,:), shift, project_size * 2, xc(i, :), yc(i, :));
        intensity(i) = CalculateIntensity(intensity(i), augment_img, shake_point(i, :), project_size); % calculating the intensity in one project size
    end
end

function augment_img = GetAugmentResImage(residual_img, est_point, project_size, xc, yc) 
    augment_img = residual_img;
    for i = 1 : 4
        for x = max(1, floor(xc(i) - project_size / 2)) : min(1024, floor(xc(i) + project_size / 2)) - 1
            for y = max(1, floor(yc(i) - project_size / 2)) : min(1024, floor(yc(i) + project_size / 2)) - 1
                [part_proj, ~, ~] = GaussianProjection(est_point, project_size, i, x, y);
...
function shake_point = Shaking(augment_img, est_point, shift, project_size, xc, yc)
    ...
    for i = 1 : 3
        ...
        residual = zeros(1,3);
        for j = 1 : 3
            residual(j) = CalculateRedidual(augment_img, new_est_point(j, :), project_size, xc, yc);
        end
...
function intensity = CalculateIntensity(intensity, augment_img, shake_point, project_size)
    % find the camera which has the highest intensity
    [project_img, xc, yc] = GaussianProjection(shake_point, project_size);
...
function residual = CalculateRedidual(augment_img, est_point, project_size, xc, yc)
    residual = 0;
    for i = 1:4
       for x = max(1, floor(xc(i) - project_size / 2)) : min(1024, floor(xc(i) + project_size / 2)) - 1 
            for y = max(1, floor(yc(i) - project_size / 2)) : min(1024, floor(yc(i) + project_size / 2)) - 1
...
function [project_img, xc, yc] = ProjectImage(est_point, project_size)
    [point_num, ~] = size(est_point);
    project_img = zeros(4, 1024, 1024);
    xc = zeros(point_num, 4); yc = zeros(point_num, 4);
    for i = 1 : point_num 
        [part_proj, xc(i, :), yc(i, :)] = GaussianProjection(est_point(i, :), project_size);
        for j = 1 : 4
            for x = max(1, floor(xc(i, j) - project_size / 2)) : min(1024, floor(xc(i, j) + project_size / 2)) - 1
                for y = max(1, floor(yc(i, j) - project_size / 2)) : min(1024, floor(yc(i, j) + project_size / 2)) - 1
...
function [project_img, xc, yc] = GaussianProjection(est_point, project_size, camid, pixel_x, pixel_y)
   calib_filepath = '/home/tanshiyong/Documents/Data/Single-Phase/11.03.17/Run1/VSC_Calib_11.02.17.mat';
   load(calib_filepath);
