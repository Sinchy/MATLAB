...
Ximg(:,1) = Ximg(:,1) - (camParaknown.Npixw - imgsize(1))/2; % particle coordinates on image plane (in pixels)
Ximg(:,2) = Ximg(:,2) - (camParaknown.Npixh - imgsize(2))/2; % particle coordinates on image plane (in pixels)

% only leave particles within the image
ind = (Ximg(:,1) >= 0) & (Ximg(:,1) <= imgsize(1)) & (Ximg(:,2) >= 0) & (Ximg(:,2) <= imgsize(2));
Ximg = Ximg(ind, :);
Xp3D = Xp3D(ind, :);

Np = size(Ximg,1); % number of particles

X = Ximg(:,1) - camParaCalib.Noffw - imgsize(1)/2;  % move the coordinate original center to the image center
Y = imgsize(2)/2 - camParaCalib.Noffh - Ximg(:,2);  % image y-axis is pointing downward, now change it to normal direction

% check if calibration points are co-planar or not

if sum(abs(Xp3D(:,1))) == 0 % co-planar case: (assume the calibration mask lies on plane xw = 0)
    % the pixel size is assumed known
    X = X*camParaknown.wpix;
    Y = Y*camParaknown.hpix;
    % calculate radial distance from the image center, in pixel units, to be used later for compute distortion coefficient
    rdsq = X.^2 + Y.^2;     
...
% In Tsai's model, tangential distortion is ignored. Here p1 and p2 are kept for compatability with other models.
camParaCalib.p1 = 0;
camParaCalib.p1star = 0;
camParaCalib.p2 = 0;
camParaCalib.p2star = 0;
