...
static void sba_crsm_build(struct sba_crsm *sm, long int *m, long int nr, long int nc);

/* allocate a sparse CRS matrix */
void sba_crsm_alloc(struct sba_crsm *sm, long int nr, long int nc, long int nnz)
{
long int msz;
  ...
  sm->nnz=nnz;
  msz=2*nnz+nr+1;
  sm->val=(long int *)malloc(msz*sizeof(long int));  /* required memory is allocated in a single step */
  if(!sm->val){
...
long int sba_crsm_elmidx(struct sba_crsm *sm, long int i, long int j)
{
register long int low, high, mid, diff;
 ...
 */
long int sba_crsm_elmidxp(struct sba_crsm *sm, long int i, long int j, long int jp, long int jpidx)
{
register long int low, high, mid, diff;
 ...
 */
long int sba_crsm_row_elmidxs(struct sba_crsm *sm, long int i, long int *vidxs, long int *jidxs)
{
 ...
 */
long int sba_crsm_col_elmidxs(struct sba_crsm *sm, long int j, long int *vidxs, long int *iidxs)
{
register long int *nextrowptr=sm->rowptr+1;
 ...
 * 0 otherwise
 */ 
long int sba_crsm_common_row(struct sba_crsm *sm, long int j, long int k)
{
register long int i, low, high, mid, diff;
