 ...
 */
struct fdj_data_x_ {
  void (*func)(double *p, struct sba_crsm *idxij, long int *rcidxs, long int *rcsubs, double *hx, void *adata); /* function to differentiate */
  long int cnp, pnp, mnp;  /* parameter numbers */
  long int *func_rcidxs,
...
inline static void *emalloc_(char *file, long int line, size_t sz)
{
void *ptr;

  ptr=(void *)mxMalloc(sz);
  if(ptr==NULL){
...
inline static void _dblzero(register double *arr, register long int count)
{
  while(--count>=0)
    *arr++=0.0;
}
...
static double sba_mean_repr_error(long int n, long int mnp, double *x, double *hx, struct sba_crsm *idxij, long int *rcidxs, long int *rcsubs)
{
...
static void sba_print_sol(long int n, long int m, double *p, long int cnp, long int pnp, double *x, long int mnp, struct sba_crsm *idxij, long int *rcidxs, long int *rcsubs)
{
register long int i, j, ii;
long int nnz;
double *ptr;
 ...
 */
static double nrmL2xmy(double *const e, const double *const x, const double *const y, const long int n)
{
const long int blocksize=8, bpwr=3; /* 8=2^3 */
register long int i;
long int j1, j2, j3, j4, j5, j6, j7;
long int blockn;
...
static double nrmCxmy(double *const e, const double *const x, const double *const y,
                      const double *const W, const long int mnp, const long int nvis)
{
const long int n=nvis*mnp;
const long int blocksize=8, bpwr=3; /* 8=2^3 */
register long int i, ii, k;
long int j1, j2, j3, j4, j5, j6, j7;
long int blockn, mnpsq;
...
static void sba_print_inf(double *hx, long int nimgs, long int mnp, struct sba_crsm *idxij, long int *rcidxs, long int *rcsubs)
{
...
static void sba_fdjac_x(
    ...
    long int    *rcidxs,           /* work array for the indexes of nonzero elements of a single sparse matrix row/column */
    long int    *rcsubs,           /* work array for the subscripts of nonzero elements in a single sparse matrix row/column */
    ...
    void   *dat)              /* I: points to a "fdj_data_x_" structure */
{
  ...
  long int n, m, nm, nnz, Asz, Bsz, ABsz, idx;
...
typedef long int (*PLS)(double *A, double *B, double *x, long int m, long int iscolmaj);
...
long int sba_motstr_levmar_x(
    const long int n,   /* number of points */
    const long int ncon,/* number of points (starting from the 1st) whose parameters should not be modified.
                   * All B_ij (see below) with i<ncon are assumed to be zero
                   */
    const long int m,   /* number of images */
    const long int mcon,/* number of images (starting from the 1st) whose parameters should not be modified.
    ...
    double *p,    /* initial parameter vector p0: (a1, ..., am, b1, ..., bn).
                   ...
                   * size m*cnp + n*pnp
                   */
    const long int cnp,/* number of parameters for ONE camera; e.g. 6 for Euclidean cameras */
    const long int pnp,/* number of parameters for ONE point; e.g. 3 for Euclidean points */
    ...
    double *covx, /* measurements covariance matrices: (Sigma_x_11, .. Sigma_x_1m, ..., Sigma_x_n1, .. Sigma_x_nm),
                   ...
                   */
    const long int mnp,/* number of parameters for EACH measurement; usually 2 */
    void (*func)(double *p, struct sba_crsm *idxij, long int *rcidxs, long int *rcsubs, double *hx, void *adata),
    ...
    const long int itmax,   /* I: maximum number of iterations. itmax==0 signals jacobian verification followed by immediate return */
    const long int verbose, /* I: verbosity */
...
{
register long int i, j, ii, jj, k, l;
long int nvis, nnz, retval;
...
/* submatrices sizes */
long int Asz, Bsz, ABsz, Usz, Vsz,
    Wsz, Ysz, esz, easz, ebsz,
    YWtsz, Wtdasz, Sblsz, covsz;

long int Sdim; /* S matrix actual dimension */
...
struct sba_crsm idxij; /* sparse matrix containing the location of x_ij in x. This is also
                        ...
                        */
long int maxCvis, /* max. of projections of a single point  across cameras, <=m */
...
register long int itno;  /* iteration counter */
long int issolved;
...
double init_p_eL2;
long int nu=2, nu2, stop=0, nfev, njev=0, nlss=0;
long int nobs, nvars;
const long int mmcon=m-mcon;
PLS linsolver=NULL;
long int (*matinv)(double *A, long int m)=NULL;
...
/* Initialization */
  ...
  S=(double *)emalloc(m*m*Sblsz*sizeof(double)*4);
  //dp=(double *)emalloc(nvars*sizeof(double));
  dp=(double *)emalloc(nvars*sizeof(double)*4);
  Wtda=(double *)emalloc(pnp*sizeof(double));
  rcidxs=(long int *)emalloc(maxCPvis*sizeof(long int));
  rcsubs=(long int *)emalloc(maxCPvis*sizeof(long int));
  ...
  if(!fjac){
    ...
    fdj_data.func_rcidxs=(long int *)emalloc(2*maxCPvis*sizeof(long int));
...
freemem_and_return: /* NOTE: this point is also reached via a goto! */
  ...
  mxFree(W);   mxFree(U);  mxFree(V);
  mxFree(e);   mxFree(eab);
  mxFree(E);   mxFree(Yj); mxFree(YWt);
  mxFree(S);   mxFree(dp); mxFree(Wtda);
  mxFree(rcidxs); mxFree(rcsubs);
#ifndef SBA_DESTROY_COVS
  if(wght) mxFree(wght);
#else
  /* nothing to do */
  ...
  mxFree(hx); mxFree(diagUV); mxFree(pdp);
  if(fdj_data.hxx){ // cleanup
    mxFree(fdj_data.hxx);
    mxFree(fdj_data.func_rcidxs);
  }
...
struct sba_crsm idxij; /* sparse matrix containing the location of x_ij in x. This is also the location of A_ij 
                        ...
                        * blocks of a sparse matrix/vector are stored
                        */
long int maxCPvis, /* max. of projections across cameras & projections across points */
    *rcidxs,  /* work array for the indexes corresponding to the nonzero elements of a single row or
                 column in a sparse matrix, size max(n, m) */
    *rcsubs;  /* work array for the subscripts of nonzero elements in a single row or column of a
                 sparse matrix, size max(n, m) */
...
freemem_and_return: /* NOTE: this point is also reached via a goto! */
  ...
  mxFree(hx); mxFree(diagU); mxFree(pdp);
  if(fdj_data.hxx){ // cleanup
    mxFree(fdj_data.hxx);
    mxFree(fdj_data.func_rcidxs);
  }
...
freemem_and_return: /* NOTE: this point is also reached via a goto! */
  ...
  mxFree(hx); mxFree(diagV); mxFree(pdp);
  if(fdj_data.hxx){ // cleanup
    mxFree(fdj_data.hxx);
    mxFree(fdj_data.func_rcidxs);
  }
