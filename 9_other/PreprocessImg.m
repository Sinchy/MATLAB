function PreprocessImg(dirr, camfoldername, image_format, save_dir, totalImgs)
...
load([save_dir 'background.mat']);

%% processing
correction = [1 1 0 0 0 0];
threshold = [0.3; 0.5; 0.5; 0.5; 0.5; 0.5];
for cam = 1:ncams
    camdir = [dirr camfoldername num2str(cam) '/'];
    % adjust threshold
%     low = 0;
...
%         img3 = img - bak(:, :, cam);
        ...
        [h,~] = imhist(img4);
        h_end = h(end);
        imwrite(img4,[camsave_dir{cam} 'cam' num2str(cam) 'frame' num2str(5,'%05.0f') '.tif']);
%          if h_end > 1
%             low = threshold(cam);
