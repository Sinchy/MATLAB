function [or3d, bad_tracks] = GetBadTracks(or3d, datapath, filterwidth, fitwidth, framerate, frame_range)
...
%         
        ...
        if good_no > no_good
            ...
            std_error = average_error + 5 * deviate_error;
        end
...
% use struct variable to make data sliced and processible by parfor
    ...
    bad_tracks = or3d(ismember(or3d(:, 5), c(delete_index(1, :) == 1)), :);
    or3d(ismember(or3d(:, 5), c(delete_index(1, :) == 1)), :) = []; % delete bad tracks
    
    save([datapath 'tracks.mat'], 'or3d', '-v7.3');
    save([datapath 'badtracks.mat'], 'bad_tracks', '-v7.3');
