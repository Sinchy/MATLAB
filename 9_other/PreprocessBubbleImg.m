...
for cam = start : ncams
    ...
    for I = start_frame : end_frame
        if (mod(I,10) == 0)
            ...
            imgdir = [camdir 'cam' num2str(cam) 'frame' num2str(I,'%05.0f') '.tif'];
            img = imread(imgdir);
...
%           fprintf(fID, ['C00' num2str(cam) 'H001S0001/' '%s\n'], a(i).name);
            fprintf(fID, [print_dir{cam} 'cam' num2str(cam) 'frame' num2str(i,'%05.0f') '.tif\n']);
%             frame = frame + 1;
% %         end
    end
    fclose(fID);
